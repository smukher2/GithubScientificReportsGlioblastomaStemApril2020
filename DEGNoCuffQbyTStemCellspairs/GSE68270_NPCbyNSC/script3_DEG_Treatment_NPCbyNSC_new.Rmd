---
title: "script3_DEG_Treatment_NPCbyNSC"
...
```{r}
#Metadata
metadata=read.csv("Step5D_result_merge_GSE68270_metadata_SampleID.csv", header=T, sep=',')

#Expression data
Expr_GeneMs=read.csv("Step5D_result_merge_GSE68270_Expr_Gene_SampleID.csv", header =T, sep=',')
...
creating_Treatment_contrasts_DEG_Limma<-makeContrasts(NPCbyNSC=TreatmentNPC-TreatmentNSC, levels =design_DEG_Limma)
...
contrast.matrix_DEG_Limma <- cbind("NPCbyNSC"=c(1,-1))
...
```{r}
#DEG_Limma for contrast "NPCbyNSC"=TreatmentNPC-TreatmentNSC 
result_NPCbyNSC_full_DEG_Limma<- topTable(eb_DEG_Limma, number = nrow(edata), adjust = "BH", coef="NPCbyNSC")
result_NPCbyNSC_full_DEG_Limma$Contrast<-rep("NPCbyNSC",nrow(result_NPCbyNSC_full_DEG_Limma))
result_NPCbyNSC_full_DEG_Limma$FC<- 2^(result_NPCbyNSC_full_DEG_Limma$logFC)
result_NPCbyNSC_FC1.25_DEG_Limma<- result_NPCbyNSC_full_DEG_Limma[which(result_NPCbyNSC_full_DEG_Limma$FC >= 1.25), ]

dim(result_NPCbyNSC_full_DEG_Limma)
dim(result_NPCbyNSC_FC1.25_DEG_Limma)


write.csv(result_NPCbyNSC_full_DEG_Limma, "Step12_result_NPCbyNSC_full_DEG_Limma.csv")
write.csv(result_NPCbyNSC_FC1.25_DEG_Limma, "Step12_result_NPCbyNSC_FC1.25_DEG_Limma.csv")

summary(result_NPCbyNSC_full_DEG_Limma)
summary(result_NPCbyNSC_FC1.25_DEG_Limma)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Limma<-rbind(result_NPCbyNSC_full_DEG_Limma) #here only one table so rbind not required
...
result_allContrasts_FC1.25_DEG_Limma<-rbind(result_NPCbyNSC_FC1.25_DEG_Limma) #here only one table so rbind not required
...
par(mfrow=c(2,2))
qqt(eb_DEG_Limma$t[,"NPCbyNSC"],df_DEG_Limma=eb_DEG_Limma$df.residual+eb_DEG_Limma$df.prior)
abline(0,1)
dev.off()
```
...
df<-result_NPCbyNSC_full_DEG_Limma
df$gene<-rownames(result_NPCbyNSC_full_DEG_Limma) #create column for genes
...
#DEG_Simple Differentially expressed genes NPCbyNSC
```{r}
#Create index to subset expression data to find the differentially expressed genes .
index_NPC=which(pheno$Treatment=='NPC')
index_NSC=which(pheno$Treatment=='NSC')
```

```{r}
#subset expression data to find the differentially expressed genes .
edata_NPCbyNSC<-edata[,c(index_NPC,index_NSC)]
head(edata_NPCbyNSC)
```

```{r}
#Differentially expressed genes between "NPCbyNSC"
t_NPCbyNSC<-apply(edata_NPCbyNSC,1,function(x){
  mod<-t.test(x[index_NPC],x[index_NSC])
  meany<-mean(2^x[index_NPC]-1)#because we had log2+1
  meanx<-mean(2^x[index_NSC]-1) #because we had log2+1
...
t_NPCbyNSC<-t(t_NPCbyNSC)
adj.P.Val<-p.adjust(t_NPCbyNSC[,'pvalue'], method="BH")
```

```{r}
#DEG_Simple for contrast "NPCbyNSC"
result_NPCbyNSC_full_DEG_Simple<-cbind(t_NPCbyNSC,adj.P.Val)
result_NPCbyNSC_full_DEG_Simple<-as.data.frame(result_NPCbyNSC_full_DEG_Simple)
cols = c("meanx","meany", "tvalue", "FC", "logFC", "pvalue", "adj.P.Val")  
result_NPCbyNSC_full_DEG_Simple[,cols] = apply(result_NPCbyNSC_full_DEG_Simple[,cols], 2, function(x) as.numeric(as.character(x)))
result_NPCbyNSC_full_DEG_Simple = result_NPCbyNSC_full_DEG_Simple[order(-result_NPCbyNSC_full_DEG_Simple$logFC), ] #order by descending fold change

result_NPCbyNSC_FC1.25_DEG_Simple<- result_NPCbyNSC_full_DEG_Simple[which(result_NPCbyNSC_full_DEG_Simple$FC >= 1.25), ]

dim(result_NPCbyNSC_full_DEG_Simple)
dim(result_NPCbyNSC_FC1.25_DEG_Simple)


write.csv(result_NPCbyNSC_full_DEG_Simple, "Step15_result_NPCbyNSC_full_DEG_Simple.csv")
write.csv(result_NPCbyNSC_FC1.25_DEG_Simple, "Step15_result_NPCbyNSC_FC1.25_DEG_Simple.csv")

summary(result_NPCbyNSC_full_DEG_Simple)
summary(result_NPCbyNSC_FC1.25_DEG_Simple)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Simple<-rbind(result_NPCbyNSC_full_DEG_Simple) #here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_Simple<-rbind(result_NPCbyNSC_FC1.25_DEG_Simple)#here only one table so rbind not required
...
df<-result_NPCbyNSC_full_DEG_Simple
df$gene<-rownames(result_NPCbyNSC_full_DEG_Simple) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
...
```{r}
#comparisons you want write here
creating_Treatment_contrasts_DEG_edgeR<-makeContrasts(NPCbyNSC=TreatmentNPC-TreatmentNSC, levels =design_DEG_edgeR)
...
contrast.matrix_DEG_edgeR <- cbind("NPCbyNSC"=c(1,-1))
...
lrt_NPCbyNSC_DEG_edgeR <- glmLRT(fit_DEG_edgeR, contrast=contrast.matrix_DEG_edgeR[,"NPCbyNSC"])

```

```{r}
#DEG_edgeR for contrast "NPCbyNSC"=TreatmentNPC-TreatmentNSC 
result_NPCbyNSC_full_DEG_edgeR<- topTags(lrt_NPCbyNSC_DEG_edgeR, n = nrow(edata), adjust.method="BH")$table
names(result_NPCbyNSC_full_DEG_edgeR)[names(result_NPCbyNSC_full_DEG_edgeR) == 'FDR'] <- 'adj.P.Val'
result_NPCbyNSC_full_DEG_edgeR$Contrast<-rep("NPCbyNSC",nrow(result_NPCbyNSC_full_DEG_edgeR))
result_NPCbyNSC_full_DEG_edgeR$FC<- 2^(result_NPCbyNSC_full_DEG_edgeR$logFC)
#drop extra genes column
result_NPCbyNSC_full_DEG_edgeR <- result_NPCbyNSC_full_DEG_edgeR[!names(result_NPCbyNSC_full_DEG_edgeR) %in% c("genes")]
result_NPCbyNSC_FC1.25_DEG_edgeR<- result_NPCbyNSC_full_DEG_edgeR[which(result_NPCbyNSC_full_DEG_edgeR$FC >= 1.25), ]

dim(result_NPCbyNSC_full_DEG_edgeR)
dim(result_NPCbyNSC_FC1.25_DEG_edgeR)


write.csv(result_NPCbyNSC_full_DEG_edgeR, "Step17_result_NPCbyNSC_full_DEG_edgeR.csv")
write.csv(result_NPCbyNSC_FC1.25_DEG_edgeR, "Step17_result_NPCbyNSC_FC1.25_DEG_edgeR.csv")

summary(result_NPCbyNSC_full_DEG_edgeR)
summary(result_NPCbyNSC_FC1.25_DEG_edgeR)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_edgeR<-rbind(result_NPCbyNSC_full_DEG_edgeR)#here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_edgeR<-rbind(result_NPCbyNSC_FC1.25_DEG_edgeR)#here only one table so rbind not required
...
df<-result_NPCbyNSC_full_DEG_edgeR
df$gene<-rownames(result_NPCbyNSC_full_DEG_edgeR) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
