---
title: "script2_Merging_Treatment_NSCbyNPC_v1"
...
#Checklist for this pipeline 
  #1) GEO series number from publication or searching GEO for dataset of interest, here GSE=GSE68270. And their respective GPL gene annotation files
  #2) metadata file made by manually combining individual metadata files from GEO to give '(original)merge_GSE68270_metadata_edit_v3' which was further edited manually to give '(manual)merge_GSE68270_metadata_edit_v3_NSCbyNPC.csv' 
...
#The individual_metadata.csv to make merged '(original)merge_GSE68270_metadata_edit_v3'
```{r}
merge_metadata=read.csv('./InputMetadata&Lists/(manual)merge_GSE68270_metadata_edit_v3_NSCbyNPC.csv', header =T, sep=',')
...
```
  ...
  #4.4 Visualization of merged GSE68270
```{r}
pdf(file="Step4_plot_Visualization_GSE68270_rawdata_transformation.pdf",height=5,width=5)
...
```
  ...
  #4.5 Visualization of merged GSE68270 few samples
```{r}
pdf(file="Step4_plot_Visualization_GSE68270_rawdata_transformation_FewSamples.pdf",height=5,width=5)
par(mfrow=c(2,1)) #plots are big, so put two per page
...
write.csv(htseqCount2_matched_tpmlog,"Step5D_result_merge_GSE68270_Expr_Gene_SampleID.csv")
write.csv(merge_metadata,"Step5D_result_merge_GSE68270_metadata_SampleID.csv")
```
