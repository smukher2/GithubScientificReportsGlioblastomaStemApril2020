---
title: "script3_DEG_Treatment_NSCbyNPC"
...
creating_Treatment_contrasts_DEG_Limma<-makeContrasts(NSCbyNPC=TreatmentNSC-TreatmentNPC, levels =design_DEG_Limma)
...
contrast.matrix_DEG_Limma <- cbind("NSCbyNPC"=c(-1,1))
...
```{r}
#DEG_Limma for contrast "NSCbyNPC"=TreatmentNSC-TreatmentNPC 
result_NSCbyNPC_full_DEG_Limma<- topTable(eb_DEG_Limma, number = nrow(edata), adjust = "BH", coef="NSCbyNPC")
result_NSCbyNPC_full_DEG_Limma$Contrast<-rep("NSCbyNPC",nrow(result_NSCbyNPC_full_DEG_Limma))
result_NSCbyNPC_full_DEG_Limma$FC<- 2^(result_NSCbyNPC_full_DEG_Limma$logFC)
result_NSCbyNPC_FC1.25_DEG_Limma<- result_NSCbyNPC_full_DEG_Limma[which(result_NSCbyNPC_full_DEG_Limma$FC >= 1.25), ]

dim(result_NSCbyNPC_full_DEG_Limma)
dim(result_NSCbyNPC_FC1.25_DEG_Limma)


write.csv(result_NSCbyNPC_full_DEG_Limma, "Step12_result_NSCbyNPC_full_DEG_Limma.csv")
write.csv(result_NSCbyNPC_FC1.25_DEG_Limma, "Step12_result_NSCbyNPC_FC1.25_DEG_Limma.csv")

summary(result_NSCbyNPC_full_DEG_Limma)
summary(result_NSCbyNPC_FC1.25_DEG_Limma)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Limma<-rbind(result_NSCbyNPC_full_DEG_Limma) #here only one table so rbind not required
...
result_allContrasts_FC1.25_DEG_Limma<-rbind(result_NSCbyNPC_FC1.25_DEG_Limma) #here only one table so rbind not required
...
par(mfrow=c(2,2))
qqt(eb_DEG_Limma$t[,"NSCbyNPC"],df_DEG_Limma=eb_DEG_Limma$df.residual+eb_DEG_Limma$df.prior)
abline(0,1)
dev.off()
```
...
df<-result_NSCbyNPC_full_DEG_Limma
df$gene<-rownames(result_NSCbyNPC_full_DEG_Limma) #create column for genes
...
#DEG_Simple Differentially expressed genes NSCbyNPC
```{r}
...
edata_NSCbyNPC<-edata[,c(index_NPC,index_NSC)]
head(edata_NSCbyNPC)
```

```{r}
#Differentially expressed genes between "NSCbyNPC"
t_NSCbyNPC<-apply(edata_NSCbyNPC,1,function(x){
  mod<-t.test(x[index_NPC],x[index_NSC])
  meany<-mean(2^x[index_NSC]-1)#because we had log2+1  
  meanx<-mean(2^x[index_NPC]-1) #because we had log2+1
...
t_NSCbyNPC<-t(t_NSCbyNPC)
adj.P.Val<-p.adjust(t_NSCbyNPC[,'pvalue'], method="BH")
```

```{r}
#DEG_Simple for contrast "NSCbyNPC"
result_NSCbyNPC_full_DEG_Simple<-cbind(t_NSCbyNPC,adj.P.Val)
result_NSCbyNPC_full_DEG_Simple<-as.data.frame(result_NSCbyNPC_full_DEG_Simple)
cols = c("meanx","meany", "tvalue", "FC", "logFC", "pvalue", "adj.P.Val")  
result_NSCbyNPC_full_DEG_Simple[,cols] = apply(result_NSCbyNPC_full_DEG_Simple[,cols], 2, function(x) as.numeric(as.character(x)))
result_NSCbyNPC_full_DEG_Simple = result_NSCbyNPC_full_DEG_Simple[order(-result_NSCbyNPC_full_DEG_Simple$logFC), ] #order by descending fold change

result_NSCbyNPC_FC1.25_DEG_Simple<- result_NSCbyNPC_full_DEG_Simple[which(result_NSCbyNPC_full_DEG_Simple$FC >= 1.25), ]

dim(result_NSCbyNPC_full_DEG_Simple)
dim(result_NSCbyNPC_FC1.25_DEG_Simple)


write.csv(result_NSCbyNPC_full_DEG_Simple, "Step15_result_NSCbyNPC_full_DEG_Simple.csv")
write.csv(result_NSCbyNPC_FC1.25_DEG_Simple, "Step15_result_NSCbyNPC_FC1.25_DEG_Simple.csv")

summary(result_NSCbyNPC_full_DEG_Simple)
summary(result_NSCbyNPC_FC1.25_DEG_Simple)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Simple<-rbind(result_NSCbyNPC_full_DEG_Simple) #here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_Simple<-rbind(result_NSCbyNPC_FC1.25_DEG_Simple)#here only one table so rbind not required
...
df<-result_NSCbyNPC_full_DEG_Simple
df$gene<-rownames(result_NSCbyNPC_full_DEG_Simple) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
...
```{r}
#comparisons you want write here
creating_Treatment_contrasts_DEG_edgeR<-makeContrasts(NSCbyNPC=TreatmentNSC-TreatmentNPC, levels =design_DEG_edgeR)
...
contrast.matrix_DEG_edgeR <- cbind("NSCbyNPC"=c(-1,1))
...
lrt_NSCbyNPC_DEG_edgeR <- glmLRT(fit_DEG_edgeR, contrast=contrast.matrix_DEG_edgeR[,"NSCbyNPC"])

```

```{r}
#DEG_edgeR for contrast "NSCbyNPC"=TreatmentNSC-TreatmentNPC 
result_NSCbyNPC_full_DEG_edgeR<- topTags(lrt_NSCbyNPC_DEG_edgeR, n = nrow(edata), adjust.method="BH")$table
names(result_NSCbyNPC_full_DEG_edgeR)[names(result_NSCbyNPC_full_DEG_edgeR) == 'FDR'] <- 'adj.P.Val'
result_NSCbyNPC_full_DEG_edgeR$Contrast<-rep("NSCbyNPC",nrow(result_NSCbyNPC_full_DEG_edgeR))
result_NSCbyNPC_full_DEG_edgeR$FC<- 2^(result_NSCbyNPC_full_DEG_edgeR$logFC)
#drop extra genes column
result_NSCbyNPC_full_DEG_edgeR <- result_NSCbyNPC_full_DEG_edgeR[!names(result_NSCbyNPC_full_DEG_edgeR) %in% c("genes")]
result_NSCbyNPC_FC1.25_DEG_edgeR<- result_NSCbyNPC_full_DEG_edgeR[which(result_NSCbyNPC_full_DEG_edgeR$FC >= 1.25), ]

dim(result_NSCbyNPC_full_DEG_edgeR)
dim(result_NSCbyNPC_FC1.25_DEG_edgeR)


write.csv(result_NSCbyNPC_full_DEG_edgeR, "Step17_result_NSCbyNPC_full_DEG_edgeR.csv")
write.csv(result_NSCbyNPC_FC1.25_DEG_edgeR, "Step17_result_NSCbyNPC_FC1.25_DEG_edgeR.csv")

summary(result_NSCbyNPC_full_DEG_edgeR)
summary(result_NSCbyNPC_FC1.25_DEG_edgeR)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_edgeR<-rbind(result_NSCbyNPC_full_DEG_edgeR)#here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_edgeR<-rbind(result_NSCbyNPC_FC1.25_DEG_edgeR)#here only one table so rbind not required
...
df<-result_NSCbyNPC_full_DEG_edgeR
df$gene<-rownames(result_NSCbyNPC_full_DEG_edgeR) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
