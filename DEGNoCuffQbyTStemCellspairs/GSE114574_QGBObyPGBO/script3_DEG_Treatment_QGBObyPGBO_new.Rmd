---
title: "script3_DEG_Treatment_QGBObyPGBO"
...
```{r}
#Metadata
metadata=read.csv("Step5D_result_merge_GSE114574_metadata_SampleID.csv", header=T, sep=',')

#Expression data
Expr_GeneHu=read.csv("Step5D_result_merge_GSE114574_Expr_Gene_SampleID.csv", header =T, sep=',')
...
DT::datatable(summary(edata_pca_unscaled[1:6,1:6]))
...
creating_Treatment_contrasts_DEG_Limma<-makeContrasts(QGBObyPGBO=TreatmentQGBO-TreatmentPGBO, levels =design_DEG_Limma)
...
contrast.matrix_DEG_Limma <- cbind("QGBObyPGBO"=c(-1,1))
...
```{r}
#DEG_Limma for contrast "QGBObyPGBO"=TreatmentQGBO-TreatmentPGBO 
result_QGBObyPGBO_full_DEG_Limma<- topTable(eb_DEG_Limma, number = nrow(edata), adjust = "BH", coef="QGBObyPGBO")
result_QGBObyPGBO_full_DEG_Limma$Contrast<-rep("QGBObyPGBO",nrow(result_QGBObyPGBO_full_DEG_Limma))
result_QGBObyPGBO_full_DEG_Limma$FC<- 2^(result_QGBObyPGBO_full_DEG_Limma$logFC)
result_QGBObyPGBO_FC1.25_DEG_Limma<- result_QGBObyPGBO_full_DEG_Limma[which(result_QGBObyPGBO_full_DEG_Limma$FC >= 1.25), ]

dim(result_QGBObyPGBO_full_DEG_Limma)
dim(result_QGBObyPGBO_FC1.25_DEG_Limma)


write.csv(result_QGBObyPGBO_full_DEG_Limma, "Step12_result_QGBObyPGBO_full_DEG_Limma.csv")
write.csv(result_QGBObyPGBO_FC1.25_DEG_Limma, "Step12_result_QGBObyPGBO_FC1.25_DEG_Limma.csv")

summary(result_QGBObyPGBO_full_DEG_Limma)
summary(result_QGBObyPGBO_FC1.25_DEG_Limma)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Limma<-rbind(result_QGBObyPGBO_full_DEG_Limma) #here only one table so rbind not required
...
result_allContrasts_FC1.25_DEG_Limma<-rbind(result_QGBObyPGBO_FC1.25_DEG_Limma) #here only one table so rbind not required
...
par(mfrow=c(2,2))
qqt(eb_DEG_Limma$t[,"QGBObyPGBO"],df_DEG_Limma=eb_DEG_Limma$df.residual+eb_DEG_Limma$df.prior)
abline(0,1)
dev.off()
```
...
df<-result_QGBObyPGBO_full_DEG_Limma
df$gene<-rownames(result_QGBObyPGBO_full_DEG_Limma) #create column for genes
...
DT::datatable(edata_keepDEG_Limma[1:6,1:6])
...
DT::datatable(summary(edata_keepDEG_Limma[1:6,1:6]))
...
DT::datatable(t_edata_keepDEG_Limma[,1:6])
```
...
#DEG_Simple Differentially expressed genes QGBObyPGBO
```{r}
#Create index to subset expression data to find the differentially expressed genes .
index_PGBO=which(pheno$Treatment=='PGBO')
index_QGBO=which(pheno$Treatment=='QGBO')
```

```{r}
#subset expression data to find the differentially expressed genes .
edata_QGBObyPGBO<-edata[,c(index_PGBO,index_QGBO)]
head(edata_QGBObyPGBO)
```

```{r}
#Differentially expressed genes between "QGBObyPGBO"
t_QGBObyPGBO<-apply(edata_QGBObyPGBO,1,function(x){
  mod<-t.test(x[index_PGBO],x[index_QGBO])
  meany<-mean(2^x[index_QGBO]-1)#because we had log2+1
  meanx<-mean(2^x[index_PGBO]-1) #because we had log2+1
...
t_QGBObyPGBO<-t(t_QGBObyPGBO)
adj.P.Val<-p.adjust(t_QGBObyPGBO[,'pvalue'], method="BH")
```

```{r}
#DEG_Simple for contrast "QGBObyPGBO"
result_QGBObyPGBO_full_DEG_Simple<-cbind(t_QGBObyPGBO,adj.P.Val)
result_QGBObyPGBO_full_DEG_Simple<-as.data.frame(result_QGBObyPGBO_full_DEG_Simple)
cols = c("meanx","meany", "tvalue", "FC", "logFC", "pvalue", "adj.P.Val")  
result_QGBObyPGBO_full_DEG_Simple[,cols] = apply(result_QGBObyPGBO_full_DEG_Simple[,cols], 2, function(x) as.numeric(as.character(x)))
result_QGBObyPGBO_full_DEG_Simple = result_QGBObyPGBO_full_DEG_Simple[order(-result_QGBObyPGBO_full_DEG_Simple$logFC), ] #order by descending fold change

result_QGBObyPGBO_FC1.25_DEG_Simple<- result_QGBObyPGBO_full_DEG_Simple[which(result_QGBObyPGBO_full_DEG_Simple$FC >= 1.25), ]

dim(result_QGBObyPGBO_full_DEG_Simple)
dim(result_QGBObyPGBO_FC1.25_DEG_Simple)


write.csv(result_QGBObyPGBO_full_DEG_Simple, "Step15_result_QGBObyPGBO_full_DEG_Simple.csv")
write.csv(result_QGBObyPGBO_FC1.25_DEG_Simple, "Step15_result_QGBObyPGBO_FC1.25_DEG_Simple.csv")

summary(result_QGBObyPGBO_full_DEG_Simple)
summary(result_QGBObyPGBO_FC1.25_DEG_Simple)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Simple<-rbind(result_QGBObyPGBO_full_DEG_Simple) #here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_Simple<-rbind(result_QGBObyPGBO_FC1.25_DEG_Simple)#here only one table so rbind not required
...
df<-result_QGBObyPGBO_full_DEG_Simple
df$gene<-rownames(result_QGBObyPGBO_full_DEG_Simple) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
...
DT::datatable(edata_keepDEG_Simple[1:6,1:6])
...
DT::datatable(summary(edata_keepDEG_Simple[1:6,1:6]))
...
DT::datatable(t_edata_keepDEG_Simple[,1:6])
```
...
```{r}
#comparisons you want write here
creating_Treatment_contrasts_DEG_edgeR<-makeContrasts(QGBObyPGBO=TreatmentQGBO-TreatmentPGBO, levels =design_DEG_edgeR)
...
contrast.matrix_DEG_edgeR <- cbind("QGBObyPGBO"=c(-1,1))
...
lrt_QGBObyPGBO_DEG_edgeR <- glmLRT(fit_DEG_edgeR, contrast=contrast.matrix_DEG_edgeR[,"QGBObyPGBO"])

```

```{r}
#DEG_edgeR for contrast "QGBObyPGBO"=TreatmentQGBO-TreatmentPGBO 
result_QGBObyPGBO_full_DEG_edgeR<- topTags(lrt_QGBObyPGBO_DEG_edgeR, n = nrow(edata), adjust.method="BH")$table
names(result_QGBObyPGBO_full_DEG_edgeR)[names(result_QGBObyPGBO_full_DEG_edgeR) == 'FDR'] <- 'adj.P.Val'
result_QGBObyPGBO_full_DEG_edgeR$Contrast<-rep("QGBObyPGBO",nrow(result_QGBObyPGBO_full_DEG_edgeR))
result_QGBObyPGBO_full_DEG_edgeR$FC<- 2^(result_QGBObyPGBO_full_DEG_edgeR$logFC)
#drop extra genes column
result_QGBObyPGBO_full_DEG_edgeR <- result_QGBObyPGBO_full_DEG_edgeR[!names(result_QGBObyPGBO_full_DEG_edgeR) %in% c("genes")]
result_QGBObyPGBO_FC1.25_DEG_edgeR<- result_QGBObyPGBO_full_DEG_edgeR[which(result_QGBObyPGBO_full_DEG_edgeR$FC >= 1.25), ]

dim(result_QGBObyPGBO_full_DEG_edgeR)
dim(result_QGBObyPGBO_FC1.25_DEG_edgeR)


write.csv(result_QGBObyPGBO_full_DEG_edgeR, "Step17_result_QGBObyPGBO_full_DEG_edgeR.csv")
write.csv(result_QGBObyPGBO_FC1.25_DEG_edgeR, "Step17_result_QGBObyPGBO_FC1.25_DEG_edgeR.csv")

summary(result_QGBObyPGBO_full_DEG_edgeR)
summary(result_QGBObyPGBO_FC1.25_DEG_edgeR)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_edgeR<-rbind(result_QGBObyPGBO_full_DEG_edgeR)#here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_edgeR<-rbind(result_QGBObyPGBO_FC1.25_DEG_edgeR)#here only one table so rbind not required
...
df<-result_QGBObyPGBO_full_DEG_edgeR
df$gene<-rownames(result_QGBObyPGBO_full_DEG_edgeR) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
...
DT::datatable(edata_keepDEG_edgeR[1:6,1:6])
...
DT::datatable(summary(edata_keepDEG_edgeR[1:6,1:6]))
...
DT::datatable(t_edata_keepDEG_edgeR[,1:6])
```
...
DT::datatable(edata_keepDEG_AllDEGmethods[1:6,1:6])
...
DT::datatable(summary(edata_keepDEG_AllDEGmethods[1:6,1:6]))
...
DT::datatable(t_edata_keepDEG_AllDEGmethods[,1:6])
```
