---
title: "script3_DEG_Treatment_PSVZbyQSVZ"
...
To reuse this code for QSVZ datasets 
...
```{r}
#Metadata
metadata=read.csv("Step5D_result_merge_GSE99777_metadata_SampleID.csv", header=T, sep=',')

#Expression data
Expr_GeneMs=read.csv("Step5D_result_merge_GSE99777_Expr_Gene_SampleID.csv", header =T, sep=',')
...
```{r}
#select the Treatment pheno drop QSVZs
...
#Aggregade by Gender
```{r}
#select the Gender pheno drop QSVZs
...
#Aggregade by Age
```{r}
#select the Age pheno drop QSVZs
...
#Aggregade by Tissue
```{r}
#select the Tissue pheno drop QSVZs
...
creating_Treatment_contrasts_DEG_Limma<-makeContrasts(PSVZbyQSVZ=TreatmentPSVZ-TreatmentQSVZ, levels =design_DEG_Limma)
creating_Treatment_contrasts_DEG_Limma
```

```{r}
#Create contrasts to find the differentially expressed genes .
#We hold QSVZ variables and surrogate variables at 0 (NA since we are using combat)
#Here the net number of rows is same and correspond to columns in fit_DEG_Limma$coefficients
contrast.matrix_DEG_Limma <- cbind("PSVZbyQSVZ"=c(1,-1))
...
```{r}
#DEG_Limma for contrast "PSVZbyQSVZ"=TreatmentPSVZ-TreatmentQSVZ 
result_PSVZbyQSVZ_full_DEG_Limma<- topTable(eb_DEG_Limma, number = nrow(edata), adjust = "BH", coef="PSVZbyQSVZ")
result_PSVZbyQSVZ_full_DEG_Limma$Contrast<-rep("PSVZbyQSVZ",nrow(result_PSVZbyQSVZ_full_DEG_Limma))
result_PSVZbyQSVZ_full_DEG_Limma$FC<- 2^(result_PSVZbyQSVZ_full_DEG_Limma$logFC)
result_PSVZbyQSVZ_FC1.25_DEG_Limma<- result_PSVZbyQSVZ_full_DEG_Limma[which(result_PSVZbyQSVZ_full_DEG_Limma$FC >= 1.25), ]

dim(result_PSVZbyQSVZ_full_DEG_Limma)
dim(result_PSVZbyQSVZ_FC1.25_DEG_Limma)


write.csv(result_PSVZbyQSVZ_full_DEG_Limma, "Step12_result_PSVZbyQSVZ_full_DEG_Limma.csv")
write.csv(result_PSVZbyQSVZ_FC1.25_DEG_Limma, "Step12_result_PSVZbyQSVZ_FC1.25_DEG_Limma.csv")

summary(result_PSVZbyQSVZ_full_DEG_Limma)
summary(result_PSVZbyQSVZ_FC1.25_DEG_Limma)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Limma<-rbind(result_PSVZbyQSVZ_full_DEG_Limma) #here only one table so rbind not required
...
result_allContrasts_FC1.25_DEG_Limma<-rbind(result_PSVZbyQSVZ_FC1.25_DEG_Limma) #here only one table so rbind not required
...
par(mfrow=c(2,2))
qqt(eb_DEG_Limma$t[,"PSVZbyQSVZ"],df_DEG_Limma=eb_DEG_Limma$df.residual+eb_DEG_Limma$df.prior)
abline(0,1)
dev.off()
```
...
df<-result_PSVZbyQSVZ_full_DEG_Limma
df$gene<-rownames(result_PSVZbyQSVZ_full_DEG_Limma) #create column for genes
...
#DEG_Simple Differentially expressed genes PSVZbyQSVZ
```{r}
#Create index to subset expression data to find the differentially expressed genes .
index_PSVZ=which(pheno$Treatment=='PSVZ')
index_QSVZ=which(pheno$Treatment=='QSVZ')
```

```{r}
#subset expression data to find the differentially expressed genes .
edata_PSVZbyQSVZ<-edata[,c(index_PSVZ,index_QSVZ)]
head(edata_PSVZbyQSVZ)
```

```{r}
#Differentially expressed genes between "PSVZbyQSVZ"
t_PSVZbyQSVZ<-apply(edata_PSVZbyQSVZ,1,function(x){
  #mod<-t.test(x[index_PSVZ],x[index_QSVZ])
  meany<-mean(2^x[index_PSVZ]-1)#because we had log2+1
  meanx<-mean(2^x[index_QSVZ]-1) #because we had log2+1
  ...
  c(meanx=meanx,meany=meany,FC=FC,logFC=logFC, Contrast=Contrast)
})
t_PSVZbyQSVZ<-t(t_PSVZbyQSVZ)
#Error in t.test.default(x[sample_QSVZ], x[sample_PSVZ]) : data are essentially constant so commented out ttest part
#if t-test works and no error above then uncommonet line below
#adj.P.Val<-p.adjust(t_PSVZbyQSVZ[,'pvalue'], method="BH")
```

```{r}
#DEG_Simple for contrast "PSVZbyQSVZ"
result_PSVZbyQSVZ_full_DEG_Simple<-cbind(t_PSVZbyQSVZ)
#if t-test works and then uncommonet line below
#result_PSVZbyQSVZ_full_DEG_Simple<-cbind(t_PSVZbyQSVZ,adj.P.Val)
result_PSVZbyQSVZ_full_DEG_Simple<-as.data.frame(result_PSVZbyQSVZ_full_DEG_Simple)
#cols = c("meanx","meany", "tvalue", "FC", "logFC", "pvalue", "adj.P.Val")  
cols = c("meanx","meany", "FC", "logFC")
result_PSVZbyQSVZ_full_DEG_Simple[,cols] = apply(result_PSVZbyQSVZ_full_DEG_Simple[,cols], 2, function(x) as.numeric(as.character(x)))
result_PSVZbyQSVZ_full_DEG_Simple = result_PSVZbyQSVZ_full_DEG_Simple[order(-result_PSVZbyQSVZ_full_DEG_Simple$logFC), ] #order by descending fold change

result_PSVZbyQSVZ_FC1.25_DEG_Simple<- result_PSVZbyQSVZ_full_DEG_Simple[which(result_PSVZbyQSVZ_full_DEG_Simple$FC >= 1.25), ]

dim(result_PSVZbyQSVZ_full_DEG_Simple)
dim(result_PSVZbyQSVZ_FC1.25_DEG_Simple)


write.csv(result_PSVZbyQSVZ_full_DEG_Simple, "Step15_result_PSVZbyQSVZ_full_DEG_Simple.csv")
write.csv(result_PSVZbyQSVZ_FC1.25_DEG_Simple, "Step15_result_PSVZbyQSVZ_FC1.25_DEG_Simple.csv")

summary(result_PSVZbyQSVZ_full_DEG_Simple)
summary(result_PSVZbyQSVZ_FC1.25_DEG_Simple)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Simple<-rbind(result_PSVZbyQSVZ_full_DEG_Simple) #here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_Simple<-rbind(result_PSVZbyQSVZ_FC1.25_DEG_Simple)#here only one table so rbind not required
...
```{r}
df<-result_PSVZbyQSVZ_full_DEG_Simple
df$gene<-rownames(result_PSVZbyQSVZ_full_DEG_Simple) #create column for genes
#mutateddf <- mutate(df, sig=ifelse((df$adj.P.Val < 0.05 & abs(df$logFC) > 0.32), "Sig adj.P.Val&FC", "Not Sig"))
#if t-test does not work use command below
mutateddf <- mutate(df, sig=ifelse((abs(df$logFC) > 0.32), "Sig FC", "Not Sig"))
#show UP100 or  in bar plot
mutateddf_UP100 <-mutateddf[order(-mutateddf$logFC), ] #descenting order sort
mutateddf_UP100 <- mutateddf_UP100[1:100,] #top100 upregulated genes
...
#Normalization:  important to normalise RNA-seq both within and between samples. edgeR has trimmed mean of M-values (TMM) method, and QSVZ options also available. TMM normalization lessens the impact of reads falling into a small set of genes
...
#lib.size norm factor <1 indicates small number of high counts are monopolizing seq data causing QSVZ genes to be lower, so library size is scaled down. Conversely, for >1 library size is scaled up analogous to downscaling the counts. 
```
...
```{r}
#comparisons you want write here
creating_Treatment_contrasts_DEG_edgeR<-makeContrasts(PSVZbyQSVZ=TreatmentPSVZ-TreatmentQSVZ, levels =design_DEG_edgeR)
creating_Treatment_contrasts_DEG_edgeR
```

```{r}
#Create contrasts to find the differentially expressed genes .
#We hold QSVZ variables and surrogate variables at 0. 
#Here the net number of rows is same and correspond to columns in fit_DEG_edgeR$coefficients
contrast.matrix_DEG_edgeR <- cbind("PSVZbyQSVZ"=c(1,-1))
...
lrt_PSVZbyQSVZ_DEG_edgeR <- glmLRT(fit_DEG_edgeR, contrast=contrast.matrix_DEG_edgeR[,"PSVZbyQSVZ"])

```

```{r}
#DEG_edgeR for contrast "PSVZbyQSVZ"=TreatmentPSVZ-TreatmentQSVZ 
result_PSVZbyQSVZ_full_DEG_edgeR<- topTags(lrt_PSVZbyQSVZ_DEG_edgeR, n = nrow(edata), adjust.method="BH")$table
names(result_PSVZbyQSVZ_full_DEG_edgeR)[names(result_PSVZbyQSVZ_full_DEG_edgeR) == 'FDR'] <- 'adj.P.Val'
result_PSVZbyQSVZ_full_DEG_edgeR$Contrast<-rep("PSVZbyQSVZ",nrow(result_PSVZbyQSVZ_full_DEG_edgeR))
result_PSVZbyQSVZ_full_DEG_edgeR$FC<- 2^(result_PSVZbyQSVZ_full_DEG_edgeR$logFC)
#drop extra genes column
result_PSVZbyQSVZ_full_DEG_edgeR <- result_PSVZbyQSVZ_full_DEG_edgeR[!names(result_PSVZbyQSVZ_full_DEG_edgeR) %in% c("genes")]
result_PSVZbyQSVZ_FC1.25_DEG_edgeR<- result_PSVZbyQSVZ_full_DEG_edgeR[which(result_PSVZbyQSVZ_full_DEG_edgeR$FC >= 1.25), ]

dim(result_PSVZbyQSVZ_full_DEG_edgeR)
dim(result_PSVZbyQSVZ_FC1.25_DEG_edgeR)


write.csv(result_PSVZbyQSVZ_full_DEG_edgeR, "Step17_result_PSVZbyQSVZ_full_DEG_edgeR.csv")
write.csv(result_PSVZbyQSVZ_FC1.25_DEG_edgeR, "Step17_result_PSVZbyQSVZ_FC1.25_DEG_edgeR.csv")

summary(result_PSVZbyQSVZ_full_DEG_edgeR)
summary(result_PSVZbyQSVZ_FC1.25_DEG_edgeR)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_edgeR<-rbind(result_PSVZbyQSVZ_full_DEG_edgeR)#here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_edgeR<-rbind(result_PSVZbyQSVZ_FC1.25_DEG_edgeR)#here only one table so rbind not required
...
df<-result_PSVZbyQSVZ_full_DEG_edgeR
df$gene<-rownames(result_PSVZbyQSVZ_full_DEG_edgeR) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
...
file.move(list.files(pattern = '*FC1.25_DEG_Cuff.csv'), "DEG_Cuff_results")
