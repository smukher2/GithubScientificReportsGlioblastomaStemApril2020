---
title: "script2_Merging_Treatment_QNPbyTAP_v1"
...
#Checklist for this pipeline 
  #1) GEO series number from publication or searching GEO for dataset of interest, here GSE=GSE70696. And their respective GPL gene annotation files
  #2) metadata file made by manually combining individual metadata files from GEO to give '(original)merge_GSE70696_metadata_edit_v3' which was further edited manually to give '(manual)merge_GSE70696_metadata_edit_v3_QNPbyTAP.csv' 
...
colnames(htseqCount2) <- gsub("_accepted_hits_PE.txt","",colnames(htseqCount2))
```
...
#The individual_metadata.csv to make merged '(original)merge_GSE70696_metadata_edit_v3'
```{r}
merge_metadata=read.csv('./InputMetadata&Lists/(manual)merge_GSE70696_metadata_edit_v3_QNPbyTAP.csv', header =T, sep=',')
...
```{r}
#Import gtf file
GTFfile = "Rattus_norvegicus.Rnor_6.0.95.gtf"
#leave feature type blank to import all features
GTF <- import.gff(GTFfile, format="gtf", genome="Rnor_6.0.95", feature.type="gene")
```
...
```{r}
write.csv(gene_length_output_2, file = "Rattus_norvegicus.Rnor_6.0.95_gene_name_length.csv")
```
...
DT::datatable(htseqCount2_keepcpm[1:4,1:4])
```

```{r}
#log2+1 transform cpm
htseqCount2_cpmlog=log2(htseqCount2_keepcpm+1)
DT::datatable(htseqCount2_cpmlog[1:4,1:4])
```
...
DT::datatable(htseqCount2_matched_tpm[1:4,1:4])
```
...
DT::datatable(htseqCount2_matched_tpmlog[1:4,1:4])
```
...
DT::datatable(htseqCount2_matched_rpkm[1:4,1:4])
```
...
DT::datatable(htseqCount2_matched_rpkmlog[1:4,1:4])
```
...
```
  ...
  #4.4 Visualization of merged GSE70696
```{r}
pdf(file="Step4_plot_Visualization_GSE70696_rawdata_transformation.pdf",height=5,width=5)
...
```
  ...
  #4.5 Visualization of merged GSE70696 few samples
```{r}
pdf(file="Step4_plot_Visualization_GSE70696_rawdata_transformation_FewSamples.pdf",height=5,width=5)
par(mfrow=c(2,1)) #plots are big, so put two per page

#randomly select 4 samples to visualize change in exprssion data with normalization. Fro large datasets to visualiza all samplse is too many and hard to see
col_sel=sample(ncol(htseqCount2), 4) #use number less than 4 is total samples is less than 4
...
write.csv(htseqCount2_matched_tpmlog,"Step5D_result_merge_GSE70696_Expr_Gene_SampleID.csv")
write.csv(merge_metadata,"Step5D_result_merge_GSE70696_metadata_SampleID.csv")
```
