---
title: "script3_DEG_Treatment_QSVZbyPSVZ"
...
To reuse this code for PSVZ datasets 
...
```{r}
#select the Treatment pheno drop PSVZs
...
#Aggregade by Gender
```{r}
#select the Gender pheno drop PSVZs
...
#Aggregade by Age
```{r}
#select the Age pheno drop PSVZs
...
#Aggregade by Tissue
```{r}
#select the Tissue pheno drop PSVZs
...
creating_Treatment_contrasts_DEG_Limma<-makeContrasts(QSVZbyPSVZ=TreatmentQSVZ-TreatmentPSVZ, levels =design_DEG_Limma)
creating_Treatment_contrasts_DEG_Limma
```

```{r}
#Create contrasts to find the differentially expressed genes .
#We hold PSVZ variables and surrogate variables at 0 (NA since we are using combat)
#Here the net number of rows is same and correspond to columns in fit_DEG_Limma$coefficients
contrast.matrix_DEG_Limma <- cbind("QSVZbyPSVZ"=c(-1,1))
...
```{r}
#DEG_Limma for contrast "QSVZbyPSVZ"=TreatmentQSVZ-TreatmentPSVZ 
result_QSVZbyPSVZ_full_DEG_Limma<- topTable(eb_DEG_Limma, number = nrow(edata), adjust = "BH", coef="QSVZbyPSVZ")
result_QSVZbyPSVZ_full_DEG_Limma$Contrast<-rep("QSVZbyPSVZ",nrow(result_QSVZbyPSVZ_full_DEG_Limma))
result_QSVZbyPSVZ_full_DEG_Limma$FC<- 2^(result_QSVZbyPSVZ_full_DEG_Limma$logFC)
result_QSVZbyPSVZ_FC1.25_DEG_Limma<- result_QSVZbyPSVZ_full_DEG_Limma[which(result_QSVZbyPSVZ_full_DEG_Limma$FC >= 1.25), ]

dim(result_QSVZbyPSVZ_full_DEG_Limma)
dim(result_QSVZbyPSVZ_FC1.25_DEG_Limma)


write.csv(result_QSVZbyPSVZ_full_DEG_Limma, "Step12_result_QSVZbyPSVZ_full_DEG_Limma.csv")
write.csv(result_QSVZbyPSVZ_FC1.25_DEG_Limma, "Step12_result_QSVZbyPSVZ_FC1.25_DEG_Limma.csv")

summary(result_QSVZbyPSVZ_full_DEG_Limma)
summary(result_QSVZbyPSVZ_FC1.25_DEG_Limma)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Limma<-rbind(result_QSVZbyPSVZ_full_DEG_Limma) #here only one table so rbind not required
...
result_allContrasts_FC1.25_DEG_Limma<-rbind(result_QSVZbyPSVZ_FC1.25_DEG_Limma) #here only one table so rbind not required
...
par(mfrow=c(2,2))
qqt(eb_DEG_Limma$t[,"QSVZbyPSVZ"],df_DEG_Limma=eb_DEG_Limma$df.residual+eb_DEG_Limma$df.prior)
abline(0,1)
dev.off()
```
...
df<-result_QSVZbyPSVZ_full_DEG_Limma
df$gene<-rownames(result_QSVZbyPSVZ_full_DEG_Limma) #create column for genes
...
#DEG_Simple Differentially expressed genes QSVZbyPSVZ
```{r}
...
edata_QSVZbyPSVZ<-edata[,c(index_PSVZ,index_QSVZ)]
head(edata_QSVZbyPSVZ)
```

```{r}
#Differentially expressed genes between "QSVZbyPSVZ"
t_QSVZbyPSVZ<-apply(edata_QSVZbyPSVZ,1,function(x){
  #mod<-t.test(x[index_PSVZ],x[index_QSVZ])
  meany<-mean(2^x[index_QSVZ]-1)#because we had log2+1
  meanx<-mean(2^x[index_PSVZ]-1) #because we had log2+1
...
t_QSVZbyPSVZ<-t(t_QSVZbyPSVZ)
#Error in t.test.default(x[sample_PSVZ], x[sample_QSVZ]) : data are essentially constant so commented out ttest part
#if t-test works and no error above then uncommonet line below
#adj.P.Val<-p.adjust(t_QSVZbyPSVZ[,'pvalue'], method="BH")
```

```{r}
#DEG_Simple for contrast "QSVZbyPSVZ"
result_QSVZbyPSVZ_full_DEG_Simple<-cbind(t_QSVZbyPSVZ)
#if t-test works and then uncommonet line below
#result_QSVZbyPSVZ_full_DEG_Simple<-cbind(t_QSVZbyPSVZ,adj.P.Val)
result_QSVZbyPSVZ_full_DEG_Simple<-as.data.frame(result_QSVZbyPSVZ_full_DEG_Simple)
#cols = c("meanx","meany", "tvalue", "FC", "logFC", "pvalue", "adj.P.Val")  
cols = c("meanx","meany", "FC", "logFC")
result_QSVZbyPSVZ_full_DEG_Simple[,cols] = apply(result_QSVZbyPSVZ_full_DEG_Simple[,cols], 2, function(x) as.numeric(as.character(x)))
result_QSVZbyPSVZ_full_DEG_Simple = result_QSVZbyPSVZ_full_DEG_Simple[order(-result_QSVZbyPSVZ_full_DEG_Simple$logFC), ] #order by descending fold change

result_QSVZbyPSVZ_FC1.25_DEG_Simple<- result_QSVZbyPSVZ_full_DEG_Simple[which(result_QSVZbyPSVZ_full_DEG_Simple$FC >= 1.25), ]

dim(result_QSVZbyPSVZ_full_DEG_Simple)
dim(result_QSVZbyPSVZ_FC1.25_DEG_Simple)


write.csv(result_QSVZbyPSVZ_full_DEG_Simple, "Step15_result_QSVZbyPSVZ_full_DEG_Simple.csv")
write.csv(result_QSVZbyPSVZ_FC1.25_DEG_Simple, "Step15_result_QSVZbyPSVZ_FC1.25_DEG_Simple.csv")

summary(result_QSVZbyPSVZ_full_DEG_Simple)
summary(result_QSVZbyPSVZ_FC1.25_DEG_Simple)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Simple<-rbind(result_QSVZbyPSVZ_full_DEG_Simple) #here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_Simple<-rbind(result_QSVZbyPSVZ_FC1.25_DEG_Simple)#here only one table so rbind not required
...
```{r}
df<-result_QSVZbyPSVZ_full_DEG_Simple
df$gene<-rownames(result_QSVZbyPSVZ_full_DEG_Simple) #create column for genes
#mutateddf <- mutate(df, sig=ifelse((df$adj.P.Val < 0.05 & abs(df$logFC) > 0.32), "Sig adj.P.Val&FC", "Not Sig"))
...
#Normalization:  important to normalise RNA-seq both within and between samples. edgeR has trimmed mean of M-values (TMM) method, and PSVZ options also available. TMM normalization lessens the impact of reads falling into a small set of genes
...
#lib.size norm factor <1 indicates small number of high counts are monopolizing seq data causing PSVZ genes to be lower, so library size is scaled down. Conversely, for >1 library size is scaled up analogous to downscaling the counts. 
```
...
```{r}
#comparisons you want write here
creating_Treatment_contrasts_DEG_edgeR<-makeContrasts(QSVZbyPSVZ=TreatmentQSVZ-TreatmentPSVZ, levels =design_DEG_edgeR)
creating_Treatment_contrasts_DEG_edgeR
```

```{r}
#Create contrasts to find the differentially expressed genes .
#We hold PSVZ variables and surrogate variables at 0. 
#Here the net number of rows is same and correspond to columns in fit_DEG_edgeR$coefficients
contrast.matrix_DEG_edgeR <- cbind("QSVZbyPSVZ"=c(-1,1))
...
lrt_QSVZbyPSVZ_DEG_edgeR <- glmLRT(fit_DEG_edgeR, contrast=contrast.matrix_DEG_edgeR[,"QSVZbyPSVZ"])

```

```{r}
#DEG_edgeR for contrast "QSVZbyPSVZ"=TreatmentQSVZ-TreatmentPSVZ 
result_QSVZbyPSVZ_full_DEG_edgeR<- topTags(lrt_QSVZbyPSVZ_DEG_edgeR, n = nrow(edata), adjust.method="BH")$table
names(result_QSVZbyPSVZ_full_DEG_edgeR)[names(result_QSVZbyPSVZ_full_DEG_edgeR) == 'FDR'] <- 'adj.P.Val'
result_QSVZbyPSVZ_full_DEG_edgeR$Contrast<-rep("QSVZbyPSVZ",nrow(result_QSVZbyPSVZ_full_DEG_edgeR))
result_QSVZbyPSVZ_full_DEG_edgeR$FC<- 2^(result_QSVZbyPSVZ_full_DEG_edgeR$logFC)
#drop extra genes column
result_QSVZbyPSVZ_full_DEG_edgeR <- result_QSVZbyPSVZ_full_DEG_edgeR[!names(result_QSVZbyPSVZ_full_DEG_edgeR) %in% c("genes")]
result_QSVZbyPSVZ_FC1.25_DEG_edgeR<- result_QSVZbyPSVZ_full_DEG_edgeR[which(result_QSVZbyPSVZ_full_DEG_edgeR$FC >= 1.25), ]

dim(result_QSVZbyPSVZ_full_DEG_edgeR)
dim(result_QSVZbyPSVZ_FC1.25_DEG_edgeR)


write.csv(result_QSVZbyPSVZ_full_DEG_edgeR, "Step17_result_QSVZbyPSVZ_full_DEG_edgeR.csv")
write.csv(result_QSVZbyPSVZ_FC1.25_DEG_edgeR, "Step17_result_QSVZbyPSVZ_FC1.25_DEG_edgeR.csv")

summary(result_QSVZbyPSVZ_full_DEG_edgeR)
summary(result_QSVZbyPSVZ_FC1.25_DEG_edgeR)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_edgeR<-rbind(result_QSVZbyPSVZ_full_DEG_edgeR)#here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC1.25_DEG_edgeR<-rbind(result_QSVZbyPSVZ_FC1.25_DEG_edgeR)#here only one table so rbind not required
...
df<-result_QSVZbyPSVZ_full_DEG_edgeR
df$gene<-rownames(result_QSVZbyPSVZ_full_DEG_edgeR) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
